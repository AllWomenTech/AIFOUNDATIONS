...
def metrics_train(X_train,y_train, model):
    ...
    statist_train = []
    list_metrics = [MAE_lTrain, MSE_lTrain, RMSE_lTrain, R2_lTrain,cv_scores]
    statist_train.append(list_metrics)
    statist_train = pd.DataFrame(statist_train,columns = ['MAE', 'MSE', 'RMSE', 'R2', 'CV_R2'], index = ['Train'])
...
def metrics_test(X_test,y_test, model):
    ...
    list_metrics = [MAE, MSE, RMSE, R2]
    statist_test.append(list_metrics)
    statist_test = pd.DataFrame(statist_test,columns = ['MAE', 'MSE', 'RMSE', 'R2'], index = ['Test'])
